...
const conversations = read("src-tauri/src/conversations.rs");
...
assert.match(cdp, /clear_stale_singleton_files/);
assert.doesNotMatch(cdp, /\/usr\/bin\/osascript/);
...
assert.doesNotMatch(cdp, /"Page\.reload"/);
assert.match(cdp, /evaluate_script\(&client, &bundle\)/);
assert.match(cdp, /verify_injection/);
assert.match(cdp, /spawn_injection_maintenance/);
assert.match(cdp, /push_balance\(&app, &client/);
assert.match(conversations, /"token_count"/);
assert.match(conversations, /"last_token_usage"/);
assert.match(conversations, /"usedTokens"/);
assert.match(conversations, /"contextWindow"/);
assert.match(conversations, /read_context\(""\)/);
...
const bundleSource = cdp.slice(cdp.indexOf("fn build_inject_bundle"));
const bundleBlock = bundleSource.slice(0, bundleSource.indexOf("async fn inject_bundle"));
assert.match(bundleBlock, /bridge\.js[\s\S]*balance-overlay\.js[\s\S]*context-bar\.js[\s\S]*thread-delete\.js/);
assert.doesNotMatch(bundleBlock, /usage-core|usage-badge/);
