...
CodexLink 是一款面向 Windows 用户的 Codex 第三方辅助工具。它把 Codex 安装、New API 账户登录与充值、API 导入、官方/API 切换、本地对话恢复以及 Codex 启动管理整合到同一个桌面界面中，减少重复配置和手动操作。
...
| 账户、余额与充值 | 登录 New API 账户，在软件内完成金额和支付方式选择 |
| 原生 API 导入 | 自动创建或复用九组账户 API，使用 Windows DPAPI 加密保存 |
| 官方/API 切换 | 可切换官方账号登录和多组 API，自动重启 Codex |
| 第三方 API | 已安装 CC Switch 时可选择性只读导入直连 Responses 路线 |
| 对话与项目 | API/官方切换时同步本地对话，并支持备份、恢复和侧边栏修复 |
| 上下文提示 | 在 Codex 输入栏底部显示当前上下文剩余比例 |
...
| 脱敏状态提示 | 只显示必要的操作结果，隐藏令牌、接口、路径和支付参数 |
...
3. 启动 CodexLink，根据界面提示完成 Codex 安装。
4. 登录账户后点击“导入 API”，再选择需要使用的令牌；也可打开“官方”开关使用 ChatGPT 账号登录。
...
首次使用建议按上图顺序完成配置。已经安装过 Codex 的用户可以直接跳过安装步骤。
...
- API 令牌使用独立的 DPAPI 加密库保存，不依赖 CC Switch，也不生成明文令牌库。
- 从 CC Switch 导入时仅只读扫描可直连的 Responses 路线，不修改或启动 CC Switch。
...
检查网络连接、Windows 应用部署服务和账户状态后重试。反馈问题时请勿公开账户、令牌、支付参数或本机配置。
...
2. 解压后，在 PowerShell 7 中运行 `Build-Setup.ps1`。
